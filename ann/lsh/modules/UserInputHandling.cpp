...
int user_input_handling(int argc, char* argv[]) {
    ...
    short int forBuildOnly_given = 0;

    while (cur_arg < argc) {
        if (strcmp(argv[cur_arg], "--for-build-only") == 0) {
            forBuildOnly_given = 1;
        }
        else if (strcmp(argv[cur_arg], "-d") == 0) { /* Checking for Input File */
    ...
    if (forBuildOnly_given == 1) return 1;
    return 0;
}
